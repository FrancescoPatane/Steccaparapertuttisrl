...
max_anno_mese = lapply(anni[,], function(year){paste(year, totali_annomese[grepl(year, totali_annomese[,1]),][which.max(totali_annomese[grepl(year, totali_annomese[,1]),2]),][,3])})

#mostra a video ed esporta come txt
cat("Anno Mese con maggior vendita\n")
tmp<-c("Anno Mese con maggior vendita\n")
for(i in max_anno_mese) {
	cat(i,"\n")
	tmp = append(tmp,i)
}
...
plot(xaxis, yaxis, main = "Mese maggior vendita", ylab = "Mesi", xlab = "Anni", pch=19, ylim=c(0,12))
dev.off()